...
_ContextBoxChangeEvent = wx.NewEventType()
EVT_RENDER_BOX_CHANGE = wx.PyEventBinder(_RenderBoxChangeEvent)
EVT_CONTEXT_BOX_CHANGE = wx.PyEventBinder(_ContextBoxChangeEvent)
...
ContextBoxChangeEvent = RenderBoxChangeEvent
...
class EditingStates(Enum):
    PICKING_AREA: int
    SCROLLING_AREA: int
    SCROLLING_CONTEXT: int
    SELECTING_STRUCTURE_GENERATION: int
    SELECTING_COLOR_GENERATION: int


class GenerationBehaviour(PointerBehaviour):
    ...
    def __init__(
        ...
        volume_half = [volume_size[0] // 2, volume_size[1] // 2, volume_size[2] // 2]

        self._active_selection: Optional[RenderSelectionEditable] = None
        self._active_context_selcetion: Optional[RenderSelectionEditable] = None
        self._editing: EditingStates = EditingStates.PICKING_AREA
        self._press_time = 0  # the time when the last box edit started
        self._start_point_1: NPArray2x3 = numpy.array(
            [
                [volume_half[0] - volume_size[0], 0, volume_half[2] - volume_size[2]],
                [volume_half[0] - volume_size[0], 0, volume_half[2] - volume_size[2]],
            ]
        )  # the state of the cursor when editing starts
        self._start_point_2: NPArray2x3 = numpy.array(
            [
                [volume_half[0], volume_size[1], volume_half[2]],
                [volume_half[0], volume_size[1], volume_half[2]],
            ]
        )  # the state of the cursor when editing starts
